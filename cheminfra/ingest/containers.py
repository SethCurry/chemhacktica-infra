...
def register_all_containers(session: Session):
  ...
  all_containers: list[DefaultContainer] = [
    ...
    DefaultContainer("Retro RetroSim", "retro_retrosim"),
		...
		DefaultContainer("Forward Augmented Transformer", "forward_augmented_transformer"),
  ...
  for container in all_containers:
    existing = session.query(Container).filter(Container.container_name == container.container_name).one_or_none()
    if existing:
      ...
      session.add(Container(display_name=container.display_name, container_name=container.container_name))
    session.commit()
...
def get_container_status(session: Session):
  ...
  for container in containers:
    ...
    new_status = ContainerStatus(status=got_container.status, container_id=container.id)
